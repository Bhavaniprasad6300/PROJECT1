...
sales_data = pd.read_csv(r"C:\Users\91630\OneDrive\Desktop\Project files\MINI PROJECT FILES\7. ARIMA model for vehicle sales\Tractor-Sales.csv")
sales_data.head(25)
...
prev_months_corr_data = pd.DataFrame(np.column_stack((                                                        mkt_sales_data_diff['Marketing Expense'][0:46].values,                                                        mkt_sales_data_diff['Tractor-Sales'][1:47].values)), columns=["Previous Month's Differenced Marketing Expense" ,"Differenced Tractor Sales"])
...
print("Correlation of current Months's Differenced Tractor Sales with Previous Month's Differenced Marketing Expense: {}"        .format(round(prev_months_corr_data.corr().iloc[0,1], 2)))
